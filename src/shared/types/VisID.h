...
class VisID : public EncodedString
{
 public:
  ...
  std::string LayerID() const
    { return substr( find( cSeparator ) ).substr( 1 ); }
  std::string DominatingLayerVisID() const
    { return WindowID() + cSeparator; }
  bool RefersWindow() const
    { return find( cSeparator ) == std::string::npos; }
  bool RefersLayer() const
    { return !RefersWindow(); }
