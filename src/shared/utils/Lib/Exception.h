...
#define EXCEPTION_ARG_(x)       Tiny::Exception::ToString( std::ostringstream() << std::flush << x )
#define stdexception(type,x)    (Tiny::Exception_<std::type>(EXCEPTION_ARG_(x), EXCEPTION_ARG_(EXCEPTION_CONTEXT_)))
...
#define std_exception(x)        stdexception(exception,x)
