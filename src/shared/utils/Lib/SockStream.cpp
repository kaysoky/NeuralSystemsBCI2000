...
# define SOCKCALL(x) (x);
#else
# define SOCKCALL(x) {if(x<0) throw std::runtime_error( __FILE__ ": Call failed: " #x );}
#endif
#define ABORT(x) {throw std::runtime_error( __FILE__ ": " #x );}
...
bool
streamsock::ip_compare::operator()( const string& inAddr1, const string& inAddr2 ) const
{
...
streamsock::close()
{
  while( check_result( ::closesocket( m_handle ) ) == retry )
    ;
  m_handle = INVALID_SOCKET;
  m_listening = false;
}
