...
bool TfMain::Process()
{
        ...
        if ( ! bci2000data ) bci2000data=new BCI2000DATA;
        if ( ! mem ) mem= new MEM;
...
//  going to get first path name from list !!!
      ...
      if( nfiles > 0 )
        ...
        if( InputForm->CheckSpatialFilter->Checked == true )
        {
                ...
                while( ( fscanf(lpfile,"%s",line) )!= EOF )
                {
                        ...
                        else if( strcmp(line,"Chan[" ) == 0 )
                        {
                                ...
                                bciinput->lap[current_signal][0]= atoi( l3 ) - 1;      //  -1 added 5/4/05
        ...
        for (cur_run=0; cur_run < nfiles; cur_run++)
        {
                ...
                if( bci2000data->Initialize(CurrentFile, 50000) == BCI2000ERR_NOERR )
                {
