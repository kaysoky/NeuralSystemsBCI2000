                            ...
                            const   TGUIRect    &inRect )
{
    ...
    else
    {
        ...
        PARAM_GET_NUM( inParamList, PROldStyle, oldStyle );
        if( oldStyle )
        {
            int numberOfTargets;
            ...
            else if( targetCode == numberOfTargets )
            {
                yLower = 80.0;
                yUpper = 100.0;
            }
    ...
    switch( audTaskMode )
    {
        ...
        case 1: // MIDI
            ...
            PARAM_ENABLE_SUFFIX( inParamList, PRGMTaskNote, targetCode );
            {
                ...
                taskMidiPlayer = new TMidiPlayer( gmInstrument, gmVolume, gmNote );
            }
            break;
        case 2: // WAV
            {
                ...
                taskWavePlayer = new TWavePlayer;
                if( taskWavePlayer->AttachFile( ( curPath + taskSound ).c_str() )
                      != TWavePlayer::noError )
    ...
    switch( audResultMode )
    {
        ...
        case 1: // MIDI
            ...
            PARAM_ENABLE_SUFFIX( inParamList, PRGMResultNote, targetCode );
            {
                ...
                resultMidiPlayer = new TMidiPlayer( gmInstrument, gmVolume, gmNote );
            }
            break;
        case 2: // WAV
            {
                ...
                if( continuousAudResult )
                {
                    ...
                    contResultWavePlayer = new TWavePlayer;
                    if( contResultWavePlayer->AttachFile( ( curPath + contResultSound ).c_str() )
                         != TWavePlayer::noError )
                           return presFileOpeningError;
                }
                resultWavePlayer = new TWavePlayer;
                if( resultWavePlayer->AttachFile( ( curPath + resultSound ).c_str() )
                     != TWavePlayer::noError )
                       return presFileOpeningError;
            }
            break;
