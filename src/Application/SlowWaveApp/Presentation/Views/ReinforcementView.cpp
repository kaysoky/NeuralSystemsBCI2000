...
TMidiPlayer::TMidiNote  TReinforcementView::noteSeq[] =
{
  { 0x3c, duration },
  { 0x40, duration },
  { 0x43, duration },
    ...
    case 1: // MIDI
      {
        ...
        if( midiPlayer == NULL )
            midiPlayer = new TMidiPlayer( gmRIInstrument, gmRIVolume );
      }
      break;
    case 2: // WAV
      {
        ...
        if( wavePlayer->AttachFile( ( curPath + riSoundFile ).c_str() )
              != TWavePlayer::noError )
