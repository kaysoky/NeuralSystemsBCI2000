        ...
        case 1: // MIDI
            {
                ...
                midiPlayer = new TMidiPlayer( gmFBInstrument, gmFBVolume );
            }
            break;
#ifdef USE_WAVE_SYNTH
        case 2: // continuous synth
...
TFeedbackView::DoFeedback( const TEventArgs& inArgs )
{
    ...
    if( xChannel >= 0 && xChannel < actualChannels )
        xNormalized = ( signal->GetValue( xChannel, 0 ) + 100.0 ) / 200.0;
    if( yChannel >= 0 && yChannel < actualChannels )
        yNormalized = ( signal->GetValue( yChannel, 0 ) + 100.0 ) / 200.0;
    if( bChannel >= 0 && bChannel < actualChannels )
        bNormalized = ( - signal->GetValue( bChannel, 0 ) + 100.0 ) / 200.0;
#else
    int                 actualChannels = signal->Channels;

    if( xChannel >= 0 && xChannel < actualChannels )
        xNormalized = ( signal->Value[ xChannel ][ 0 ] + 100.0 ) / 200.0;
    if( yChannel >= 0 && yChannel < actualChannels )
        yNormalized = ( signal->Value[ yChannel ][ 0 ] + 100.0 ) / 200.0;
    if( bChannel >= 0 && bChannel < actualChannels )
        bNormalized = ( - signal->Value[ bChannel ][ 0 ] + 100.0 ) / 200.0;
#endif
