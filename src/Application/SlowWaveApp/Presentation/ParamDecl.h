...
#else // BCI2000
# define PARAM_ENABLE            PARAM_ADD
# define PARAM_ENABLE_SUFFIX     PARAM_ADD_SUFFIX
# define PARAM_ENABLE_RT         PARAM_ADD_RT
#endif // BCI2000
...
#define PARAM_GET_NUM( List, Name, Variable )                       \
        {                                                           \
            ...
            if( _paramPtr == NULL ||                                \
                ( ( _valuePtr = _paramPtr->GetValue() ) == NULL ) || \
                ( *_valuePtr == '\0' ) )                            \
            {                                                       \
...
#define PARAM_GET_NUM_SUFFIX( List, Name, Suffix, Variable )        \
        {                                                           \
            ...
            if( _paramPtr != NULL )                                 \
              _valuePtr = _paramPtr->GetValue();                    \
            else                                                    \
            {                                                       \
              PARAM_GET_PTR( List, Name, _paramPtr );               \
              if( _paramPtr != NULL )                               \
                _valuePtr = _paramPtr->GetValue( Suffix );          \
            }                                                       \
            if( _paramPtr == NULL ||                                \
...
#define PARAM_GET_NUM_SUFFIX_BY_INDEX( List, Name, Suffix, Index, Variable ) \
        {                                                             \
            ...
            PARAM_GET_PTR_SUFFIX( List, Name, Suffix, _paramPtr );    \
            if( _paramPtr != NULL )                                   \
              _valuePtr = _paramPtr->GetValue( Index );               \
            else                                                      \
            {                                                         \
              PARAM_GET_PTR( List, Name, _paramPtr );                 \
              if( _paramPtr != NULL )                                 \
                _valuePtr = _paramPtr->GetValue( Suffix, Index );     \
            }                                                         \
            if( _paramPtr == NULL ||                                  \
                _valuePtr == NULL || *_valuePtr == '\0' )             \
            {                                                         \
...
#define PARAM_GET_STRING( List, Name, Variable )                    \
        {                                                           \
            ...
            PARAM_GET_PTR( List, Name, _paramPtr );                 \
            if( _paramPtr == NULL ||                                \
                ( ( Variable = _paramPtr->GetValue() ) == NULL ) || \
                ( *Variable == '\0' ) )                             \
            {                                                       \
                gPresErrors.AddError( presParamInaccessibleError,   \
                                            _param_##Name.name() ); \
...
#define PARAM_GET_STRING_SUFFIX( List, Name, Suffix, Variable )     \
        {                                                           \
            ...
            PARAM_GET_PTR_SUFFIX( List, Name, Suffix, _paramPtr );  \
            if( _paramPtr != NULL )                                 \
              Variable = _paramPtr->GetValue();                     \
            else                                                    \
            {                                                       \
              PARAM_GET_PTR( List, Name, _paramPtr );               \
              if( _paramPtr != NULL )                               \
                Variable = _paramPtr->GetValue( Suffix );           \
            }                                                       \
            if( _paramPtr == NULL ||                                \
