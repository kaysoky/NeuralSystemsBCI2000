...
\todo{Check whether there is a problem with TForms inheriting from Environment,
and if yes, present a work-around here.}
...
\texttt{GenericFilter} inherits from \texttt{SignalProperties}.
...
override their (empty) default implementation with your own version, 
depending on what your filter needs to do:
\begin{itemize}

 \item \texttt{Resting} is called instead of \texttt{Process} when the system
       is in suspended state. This is also the only function from which
       a filter may change a parameter value. Any parameter changes inside
       \texttt{Resting} will propagate to the other modules without any 
       explicit request from your side.
 ...
 \item Your filter's \textit{Destructor} should free all resources 
       ...
       In many cases, freeing of resources will be done implicitly if you use
...
-- the time constant is given in units of the sample duration, resulting in a
...
\item Input and output signal pointers are assumed to point to valid locations
      in memory.
\item The output signal is assumed to hold at least as much data as the input
...
To accomplish this, we add a member of type \texttt{GenericVisualization}
to our filter class, adding
