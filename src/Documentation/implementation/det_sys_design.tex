...
\texttt{GenericSignal} uses a \texttt{SignalProperties} member to maintain
...
Luckily, \textit{Tachyon Corporation} gives you just
                              ...
                              SignalProperties& outputProperties ) const
{
  ...
  outputProperties = SignalProperties( Parameter( "SoftwareCh" ),
                          Parameter( "SampleBlockSize" ), SignalType::int16 );
}
\end{verbatim}\codeend

Here, the last argument of the \texttt{SignalProperties} constructor
determines not only the type of the signal propagated to the BCI2000 filters 
but also the format of the \texttt{dat} file written by the source module.
...
\subsubsection{A simple low pass filter}

We want to implement a low pass filter with a time constant $T$ (given in units
...
The resulting filter class is to be called \texttt{LPFilter}.
We create two new files, \texttt{LPFilter.h}, and \texttt{LPFilter.cpp},
and put a minimal filter declaration into \texttt{LPFilter.h}:
...
Into \texttt{LPFilter.cpp} we put the lines
...
When implementing a filter, a good strategy is to begin with the \texttt{Process}
function, and to consider the remaining class member functions mere helpers, mainly
determined by the code of \texttt{Process}.
So we convert (\ref{eq:LPFormula}) into the \texttt{Process}
...
(\ref{eq:LPFormula}), we introduced member variables representing these
sub-expressions:
\begin{eqnarray*}
...
value into the LPTimeConstant parameter; we need to make sure that an error
...
operations, there is nothing to be done inside the \texttt{LPFilter} 
\textit{destructor}.
Our \textit{constructor} will contain initializers for the members we declared,
and a BCI2000 parameter definition for LPTimeConstant:
...
external" linker error that reminds us to add our \texttt{LPFilter.cpp} to that module's project.

\subsubsection{Visualizing filter output}

We would like to present the LPFilter's output signal in an operator window.
...
\texttt{LowPass} at the end of the \texttt{SOURCEID} enumeration in the file
...
#include "defines.h"
\end{verbatim}\codeend
and change the \texttt{LPFilter} constructor to read
...
version of \texttt{LPFilter} -- without visualization -- in the example.

To obtain 1-based instance numbers for the instances of \texttt{LPFilter}, 
...
\item adding the definition and initialization 
  ...
  \end{verbatim}\codeend
  to \texttt{LPFilter.cpp};
\item adding an initializer for \texttt{mParamName\_LPTimeConstant} to the
  constructor, appending the instance number to it, and replacing all 
  occurrences of the parameter name with \texttt{mParamName\_LPTimeConstant}:
...
LPFilter::~LPFilter()
{
  ...
  \end{verbatim}\codeend
\item replacing the literal occurrences of \texttt{"LPTimeConstant"} in the 
  \texttt{Preflight}, \texttt{Initialize}, and \texttt{Process} functions
  with \texttt{mParamName\_LPTimeConstant}.
\end{itemize}
...
Filter( NormalFilter, 2.E2 );
  \end{verbatim}\codeend
each instance of \texttt{LPFilter} has its own parameter
\texttt{LPTimeConstant1}, \texttt{LPTimeConstant2}, and so on, and the numbers
...
Support for various output formats is provided by ``File Writer'' classes descending 
from \texttt{GenericFileWriter}.
At runtime, selection of a file format is achieved by specifying a command line 
parameter to the source module, e.\ g.\ \\
\verb|DASSource --FileFormat-Null|.\\
The value following the last minus character, appended with \texttt{FileWriter},
will be matched against the names of
all \texttt{GenericFileWriter} descendants present in the source module.
In the example, the \texttt{NullFileWriter} class will be used for data output
(which will not produce any output file).

\subsubsection{BCIDat File Format}
Parameters, BCI2000 state variables, and brain signal data will be written into a 
BCI2000 data file
as defined in the BCI2000 specification (``Project Outline'' document).
This is also the default if no file format is selected.

\subsubsection{Null File Format}
No information is recorded. 
Individual filters may still write log files to the directory defined
by the \texttt{FileInitials}, \texttt{SubjectName}, and \texttt{SubjectSession}
parameters.

\subsubsection{EDF File Format}
EDF (``European Data Format'') is a standard for biosignal data especially popular
is sleep research.

\begin{itemize}
 \item EDF is limited to 16 bit data.
 \item BCI2000 state variables will be mapped to additional signal channels that 
       contain a single sample for each data block.
 \item BCI2000 parameters cannot be represented in EDF format.
       A separate BCI2000 parameter file may be saved along with an EDF data file.
\end{itemize}

\paragraph{EDF References} 
\begin{itemize}
 \item Bob Kemp, Alpo V\"arri, Agostinho C. Rosa, Kim D. Nielsen and John Gade 
...
Electroencephalography and Clinical Neurophysiology, 82 (1992) 391-393.
 \item \verb|http://www.hsr.nl/edf/|
\end{itemize}

\subsubsection{GDF File Format}
GDF is a general data format for biosignals which is popular in BCI research.
Building on EDF, GDF allows for arbitrary numeric data types, introduces
an event table, and provides standardized encoding of events.

\begin{itemize}
 \item Currently, BCI2000 supports Version 1.25 of the GDF format specification.
 \item BCI2000 does not prescribe the semantics of its state variables.
       Thus, a general mapping of BCI2000 states onto GDF events is not 
       possible.
       Instead, GDF events are created via a user-defined set of rules
       (predefined for the most important cases).
 \item Additionally, all BCI2000 state variables will be mapped to additional 
       signal channels the same way as for EDF.
 \item BCI2000 parameters cannot be represented in GDF format.
       A separate BCI2000 parameter file may be saved along with a GDF data file.
\end{itemize}

\paragraph{GDF References} 
\begin{itemize}
 \item A. Schl\"ogl et al, ``GDF: A General Data Format for Biosignal Data: Version 1.25'',
       \verb|http://bci.tugraz.at/~schloegl/matlab/eeg/gdf4/TR_GDF.pdf|.
 \item A. Schl\"ogl, BIOSIG, \verb|http://biosig.sf.net/|. 2003-2005.
\end{itemize}
