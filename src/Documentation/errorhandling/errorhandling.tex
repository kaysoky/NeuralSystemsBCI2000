...
%\pagebreak
%\appendix
%\section{Appendix: Coding Strategies}
%
%The following code snippets are examples of minor negligences that
%do not constitute mistakes but, in the opinion of the author,
%unnecessarily introduce complexity and/or possible error conditions
%into the resulting program.
%All examples are taken from actual code.
%
...
%}
%\end{verbatim}
%This will result in a Division by Zero runtime error if any of
%$b$ or $c$ becomes zero. However, the $c=0$
%case is perfectly valid, and by rearranging operands we obtain a
%version that will handle that case correctly:
%\begin{verbatim}
%  return a * c / b;
%\end{verbatim}
%
...
%}
%\end{verbatim}
%More or less disguised, this kind of code happens often if $b/c$ as a ratio
%constitutes an entity in its own right. However, the conversions introduce
%unneeded complexity, and numerical subtleties may lead to results difficult
%to understand.
%The intended result is obtained by writing
%\begin{verbatim}
%  return ( a * b ) / c;
%\end{verbatim}
%
%\subsection{Erring Security}
%\begin{verbatim}
%class SomeClass
%{
% private:
%  SomeOtherClass* somePointer;
%  
% public:
%  SomeClass()
%  {
...
%    if( somePointer )
%      delete somePointer;
%    somePointer = new SomeOtherClass;
%  }
%};
%\end{verbatim}
%Checking \texttt{somePointer} before deleting the object it points to
%looks good. However, deleting a \texttt{NULL} pointer does not constitute
%an error condition but simply does nothing; the \texttt{if} clause is already
%built into the \texttt{delete} operator.
%What really matters is to make sure that the pointer is zero when it is
%not pointing to a valid object. For the example class, this means that we
%properly initialize \texttt{somePointer}.
%\begin{verbatim}
%class SomeClass
%{
% private:
...
%\end{verbatim}
%
%If \texttt{SomeClass} can be accessed by multiple threads, we must make sure
%that places in the code where \texttt{somePointer} points to an invalid
%memory location are protected from interruption, using some locking mechanism:
%\begin{verbatim}
%  void SomeMemberFunction()
%  {
%    AcquireLock();
...
%  }
%\end{verbatim}
%Why shouldn't we protect the destructor by writing the following?
%\begin{verbatim}
%  ~SomeClass()
%  {
%    AcquireLock();
...
%  }
%\end{verbatim}
%The only context in which this code ever made a difference was if one thread accessed
%an instance of \texttt{SomeClass} while this instance was being deleted by another
%thread. Such a situation, however, can only be due to a really bad mistake which we
%definitely don't want to cover up that way. Instead, we do want a crash to
%tell us we got something wrong on a more fundamental level.
%
%\subsection{Tries and Catches}
%What's wrong with the following code?
%\begin{verbatim}
%try
%{
%  x = MyReadXFromFile( fileName );
%}
%catch(...)
%{
%  x = myDefaultValue;
%}
%\end{verbatim}
%The \texttt{catch} says we expect something not to work. Why don't we handle
%it inside \texttt{MyReadXFromFile()}? There may be good reasons for this. But then,
%a lot of things might go wrong inside the \texttt{try} block, most of which we
%don't expect. We might happily believe everything works fine, but there might be
%a Division By Zero exception in \texttt{MyReadXFromFile()} each time we call it,
%resulting in \texttt{x} always having \texttt{myDefaultValue}. So if we write
%\begin{verbatim}
%try
%{
%  x = MyReadXFromFile( fileName );
...
%}
%\end{verbatim}
%we know that the Division By Zero exception will show up, notifying us of a
%programming error we certainly need to fix, while any VCL exception from accessing
%the file will result in a silently applied default value.
