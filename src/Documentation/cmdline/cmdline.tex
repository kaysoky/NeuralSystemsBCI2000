...
parameter from the BCI2000 data file \texttt{mydata.dat} file, and display 
...
On the Windows NT shell, change to the \texttt{BCI2000/Tools/cmdline} directory,
and execute \texttt{make all} from there.

To use the tools from any directory, you will need to add the 
\texttt{BCI2000/Tools/cmdline} directory to your system's PATH environment variable. 
...
for the user, a typical conversion will use \textit{two} of the tools provided, 
...
\\ Only states and parameters but no data will be contained in the resulting stream.
...
Each object contained in the stream will appear as its type name, followed by an 
...
Each state will appear on its own line, thus values of certain states may be easily 
extracted using the \texttt{grep} program.

\subsubsection{\texttt{bci\_stream2table}}
\texttt{bci\_stream2table} converts a stream into a tab-separated table containing 
states and signal values in ASCII format.
...
For this to work, it is necessary that the path to the filter's \texttt{cpp}
file is contained in the makefile's \texttt{SIGPROC} variable; you may have to 
...
\verb|MyFilter --help|\\
You should get a message that the program applies your filter to its input, or 
otherwise a hint pointing you towards fixing it.
...
To extract parameters from a data file, one convert it into a stream using 
...
\texttt{bci\_dat2stream}'s transmit option to suppress parameters, and combine 
its output with \texttt{bci\_prm2stream}'s into a single stream. 
