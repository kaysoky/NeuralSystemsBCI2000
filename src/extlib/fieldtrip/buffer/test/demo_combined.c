...
int main(int argc, char *argv[]) {
	...
	rc = pthread_create(&tid, &attr, tcpserver, (void *)(&host));
	if (rc) {
		printf("ERROR1; return code from pthread_create() is %d\n", rc);
		exit(-1);
	}

	// FIXME the initialization with the multiple threads requires some more though
	usleep(1000000);

	/* simultaneous detection of events from the serial and/or parallel ports is possible if these were to run in a seperate thread */
	// serport_thread  (NULL);
	// parport_thread  (NULL);

	/* start the EEG/MEG data acquisition*/
	// biosemi_thread  (NULL);
	// brainamp_thread (NULL);
	// ctf_thread      (NULL);
	// eldith_thread   (NULL);
	sinewave_thread ((void *)(&host));
	// micromed_thread (NULL);
	// tmsi_thread     (NULL);
