...
void *tcpsocket(void *arg) {
    ...
    if (verbose>0) fprintf(stderr, "tcpsocket: client = %d\n", client);
	...
	while (1) {
		...
		if (request->def->version!=VERSION) {
			fprintf(stderr, "tcpsocket: incorrect request version\n");
			goto cleanup;
		}
		...
		if ((status = dmarequest(request, &response)) != 0) {
			fprintf(stderr, "tcpsocket: an unexpected error occurred\n");
			goto cleanup;
		}
		...
		if ((n = bufwrite(client, response->def, sizeof(messagedef_t)))!=sizeof(messagedef_t)) {
			fprintf(stderr, "tcpsocket: write size = %d, should be %d\n", n, sizeof(messagedef_t));
			goto cleanup;
		}
