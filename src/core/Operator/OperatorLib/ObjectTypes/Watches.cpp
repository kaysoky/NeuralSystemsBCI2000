  ...
  mSendMessages( &Watch::SendMessages, this )
{
  ...
  if( mID == BCI_None )
  {
    if( inAddress.empty() )
    {
      ...
      mSocket.open( addr.c_str() );
      if( mSocket.is_open() )
        mAddress = addr;
    }
    else
    {
      mSocket.open( inAddress.c_str() );
      if( mSocket.is_open() )
      {
...
{
  OSMutex::Lock lock( mCountMutex );
  mBuf = inMessage;
  char header[] = { ( mCount / 10 ) % 10 + '0', mCount % 10 + '0', '\t', 0 },
       footer[] = "\r\n";
  ++mCount %= 100;
  TemporaryLock( mQueue )().push( header + inMessage + footer );
