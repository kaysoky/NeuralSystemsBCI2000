...
void DTADC::Initialize()
{
 ...
 if ((strcmp(Parameter("BoardName2"), "") != 0) && (strcmp(Parameter("BoardName2"), "none") != 0))
    Board2Active=true;
 else
    Board2Active=false;
 ...
 if (!Board2Active)
    {
    dtfun.InitBoard(Parameter("BoardName"), false);
    dtfun.SetFunction( );
    ...
    //
    // BOARD 1
    dtfun.InitBoard(Parameter("BoardName"), true);      // do produce a clock signal on the counter/timer pin
    dtfun.SetFunction( );
    dtfun.ConfigAD(  OL_CHNT_SINGLEENDED, channelsb1, 2, 1, channelsb1 * samplerate, (UINT) channelsb1 * blocksize );
    //
    // BOARD 2
    dtfun2.InitBoard(Parameter("BoardName2"), true);       // do produce a clock signal on the counter/timer pin
    dtfun2.SetFunction( );
...
bool    test2boards;     // have to define this again, because can't access Board2Active here
  ...
  if (test2boards)
     ...
     if (Parameter( "SoftwareChBoard1" )+Parameter( "SoftwareChBoard2" ) != Parameter( "SoftwareCh" ))
        bcierr << "If we have two boards, SoftwareChBoard1+SoftwareChBoard2 has to equal SoftwareCh" << endl;
  ...
  if (!test2boards)
     ...
     && dtfun.CleanUp() == OLSUCCESS;
     // BOARD2
     dtfun2.InitBoard(Parameter( "BoardName2" ), false);
     dtfun2.SetFunction();
     board2Accessible =
     dtfun2.ConfigAD(
        OL_CHNT_SINGLEENDED,
    ...
    for (sample=0; sample<blocksize; sample++)
     ...
     for (channel=0; channel<channelsb1; channel++)
      ...
      if (channel < channelsb2)
         signal->SetValue( channel+channelsb1, sample, value );
      count++;
      }
     }
    }
