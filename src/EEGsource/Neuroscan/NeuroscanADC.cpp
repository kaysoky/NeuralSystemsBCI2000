...
float LSB;
  ...
  //
  client_tcpsocket s( Parameter("ServerAddress") );
  tcpstream server( s );
  if ( !server.is_open() )
     {
...
void NeuroscanADC::Initialize()
{
  ...
  if (c_tcpsocket) delete c_tcpsocket;
  // make the connection
  c_tcpsocket=new client_tcpsocket( Parameter("ServerAddress") );
  server=new tcpstream( *c_tcpsocket );
  if ( !server )
     {
