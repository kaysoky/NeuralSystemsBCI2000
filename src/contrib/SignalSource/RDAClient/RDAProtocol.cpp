...
Connection::Open( const string& inHostName )
{
  ...
  { // RDA example code suggests to use an external address rather than localhost
    // when connecting.
    hostName = *streamsock::local_addresses().rbegin();
  ...
  else
  {
    ...
    if( h.type != RDAStart || !mInfo.Get( mStream ) )
      bcierr << "Error receiving start block";
    else
    {
...
{
  bciassert( Output.Channels() > static_cast<int>( mInfo.numChannels ) );
  bool done = false;
