...
void Enobio_ADC::Initialize( const SignalProperties&, const SignalProperties& )
{
	...
	mSignalBuffer=(int*) malloc(mSourceCh*sizeof(int)*mSampleBlockSize);
}
...
//	bciout<<"on adc process"<<endl;
	...
	while(count<mSampleBlockSize&&err.empty()){
		count=dataHandler.consumeData(mSignalBuffer,mSampleBlockSize,count);
	...
	for(int sample=0;sample<count;++sample){
		for (int channel=0;channel<mSourceCh;++channel){
			outputSignal( channel, sample )=mSignalBuffer[mSourceCh*sample+channel];
		}
	}
}
