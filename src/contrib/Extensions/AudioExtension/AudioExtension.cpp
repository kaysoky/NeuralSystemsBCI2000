...
void AudioExtension::Publish()
{
  ...
  {
    bcidbg( 0 ) << "    " << Pa_GetHostApiInfo( i )->name
  ...
  bcidbg( 0 ) << "AudioExtension enabled using " << Pa_GetVersionText()  << endl;
}
...
void AudioExtension::Preflight() const
{
  ...
  bcidbg( 0 ) << apiInfo->name << " was requested as the host API" << endl;
  bcidbg( 0 ) << "HostAPI: " << apiInfo->name << " supports the following "
  ...
  for( int i = 0; i < apiInfo->deviceCount; i++ )
  {
    ...
    bcidbg( 0 ) << "    Device " << i << ": " << info->name << " Supports " << info->maxInputChannels
  ...
  bcidbg( 0 ) << "Device configuration supports " << inputChannels 
  ...
  int &inputIdx, int &outputIdx, int &inputChannels, int &outputChannels ) const
{
  ...
  {
    if( inputIdx == -1 )
      bcidbg( 0 ) << "Error retrieving default input device on requested Host API" << endl;
    else
      bcidbg( 0 ) << "Error retrieving input device " << inputIdx
                  << " on requested Host API; are you sure it's valid?" << endl;
  }
  if( !outDevInfo )
  {
    if( outputIdx == -1 )
      bcidbg( 0 ) << "Error retrieving default output device on requested Host API" << endl;
    else
      bcidbg( 0 ) << "Error retrieving output device " << outputIdx
                  << " on requested Host API; are you sure it's valid?" << endl;
  }
