////////////////////////////////////////////////////////////////////////////////
// $Id: SimulBCI.cpp 4008 2012-05-15 12:42:40Z dsarma $
//
////////////////////////////////////////////////////////////////////////////////
#include "PCHIncludes.h"
#pragma hdrstop

#include "SimulBCITask.h"
...
//      filename = FileUtils::AbsolutePath( filename );
  ...
  //Parameter( "VisualFeedback" );
  //Parameter( "TactileFeedback" );
  //Parameter( "MaxValue" );
  //Parameter( "MinValue" );
  //Parameter( "HeldGain" );
  //Parameter( "HeldFrequency" );
  ...
  //if (Parameter( "TactileFeedback" ) == 1) {
	 // ParamRef tactileCatch = Parameter( "TactileCatchTrials" );
	 // for( int i = 0; i < tactileCatch->NumValues(); ++i )
		//if( tactileCatch( i ) < 1 )
		//  bcierr << "Invalid stimulus code "
		//		 << "(" << tactileCatch( i ) << ") "
		//		 << "at tactileCatch(" << i << ")"
	 ...
	 // Parameter( "TactorBoardComPort" );
	 // int modType = Parameter( "TactileFeedbackModulationType" );

	  //if (modType != 0 && modType != 1)
		 // bcierr << "Invalid modulation type, must be 0 or 1" << endl;


	  //// Make sure we can connect to tactor board manager
	  //TactorManager* manager;
	  //bciout << "Initializing connection to tactor board, this may take a few seconds" << endl;
	  //manager = manager->getInstance();

	  //if (manager == NULL) {
		 // bcierr << "Unable to instantiate tactor Manager" << endl;
	  //}
	  //else {
		 // if (manager->getNumDiscoveredDevices() <= 0) {
			//  bcierr << "No tactor devices found: No tactor board connected, or problems with tactor board driver." << endl;
		 // }
	  //}
  //}
}
...
//  else
  ...
  //mTactileFeedback = ( Parameter( "TactileFeedback" ) == 1);

  //mMaxValue = Parameter( "MaxValue" );
  //mMinValue = Parameter( "MinValue" );
  //mHeldGain = Parameter( "HeldGain" );
  //mHeldFrequency = Parameter( "HeldFrequency" );
  ...
  //if (mTactileFeedback == true) {
	 // // Get connection to our tactor board of choice
	 // TactorManager* manager;

	 // manager = manager->getInstance();
	 // 
	 // mTactorBoard = manager->getTactorBoardByName( Parameter( "TactorBoardComPort" ).c_str() );

	 // if (mTactorBoard == NULL) {
		//  int deviceCount = manager->getNumDiscoveredDevices();
		//  if (deviceCount <= 0) {
		//	  bcierr << "No tactor devices found: No tactor board connected, or problems with tactor board driver." << endl;
		//  }
		//  else {
		//	  bcierr << "No tactor device found at requested COM port, but there are other COM devices at the following ports, are you sure your parameter file has the correct COM port listed?" << endl;
		//	
		//	  for (int i = 0; i < deviceCount; i++) {
		//		  TactorBoard * board = manager->getTactorBoardByIndex(i);
		//		  bcierr << "  " << board->getName() << endl;
		//		  delete board;
		//	  }
		//  }
	 // }
	 // else {
		//mTactileCatchTrials.clear();
  //      for( int j = 0; j < Parameter( "TactileCatchTrials" )->NumValues(); ++j )
	 //       mTactileCatchTrials.push_back( Parameter( "TactileCatchTrials" )( j ) );
	 // }
  //}

  //mFeedbackModulationType = Parameter( "TactileFeedbackModulationType" );
  //
  //enum { intensity, frequency };

  //// TODO handle settable max/min
  //if (mFeedbackModulationType == frequency) {
	 // mTactorVarDelta = (mMaxValue - mMinValue) / 2 / feedbackDuration;
  //}
  //else {
	 // mTactorVarDelta = (mMaxValue - mMinValue) / 2 / feedbackDuration;
  //}
...
//  mTactorBoard->connect();
//
//  enum { intensity, frequency };
//
//  // TODO handle settable max/min
//  if (mFeedbackModulationType == frequency) {
////	  mTactorBoard->setGain((int) floor((float)((mMaxValue - mMinValue) / 2) + mMinValue));
//	  AppLog << "Setting Gain to : " << mHeldGain << endl;
//	  mTactorBoard->setGain(mHeldGain);
//  }
//  else {
////	  mTactorBoard->setSin1Frequency((int) floor((float)((mMaxValue - mMinValue) / 2) + mMinValue));
//	  AppLog << "Setting Frequency to : " << mHeldFrequency << endl;
//	  mTactorBoard->setSin1Frequency(mHeldFrequency);
//  }
...
DynamicFeedbackTask::OnTrialBegin()
{
  ...
  /*if (mTactileFeedback == true) {
	  mIsTactileCatchTrial = false;
	  for (size_t i = 0; i < mTactileCatchTrials.size(); i++)
		  mIsTactileCatchTrial = (mTactileCatchTrials.at(i) == mTrialCount);

	  if (mIsTactileCatchTrial == true) {
		  AppLog.Screen << "<- tactile catch trial" << endl;
	  }*/
 /* }*/
...
DynamicFeedbackTask::OnFeedbackBegin()
{
  ...
  //if (mTactileFeedback == true) {
	 ...
	 // if (mFeedbackModulationType == frequency) { 
		//  mTactorVar = ((mMaxValue - mMinValue) / 2) + mMinValue;
		//  mTactorBoard->setSin1Frequency((int) floor(mTactorVar));
	 // }
	 // else {
		//  mTactorVar = ((mMaxValue - mMinValue) / 2) + mMinValue;
		//  mTactorBoard->setGain((int) floor(mTactorVar));
	 // }
  //}

  //if (mTactileFeedback == true && mIsTactileCatchTrial == false)
	 // // TODO make tactor(s) a selection
	 // mTactorBoard->tact(TAC1);
...
DynamicFeedbackTask::DoFeedback( const GenericSignal& ControlSignal, bool& doProgress )
{
	...
	/*enum { intensity, frequency };
	if(mTactileFeedback == true && mIsTactileCatchTrial == false) {
		mTactorVar += mTactorVarDelta * ControlSignal ( 0, 0 );
		
		if (mFeedbackModulationType == frequency) {
			if (mTactorVar > mMaxValue) mTactorVar = mMaxValue;
			if (mTactorVar < mMinValue) mTactorVar = mMinValue;
			...
			AppLog << "Setting intensity: " << (int) floor(mTactorVar);
			int result = mTactorBoard->setGain((int) floor(mTactorVar));
			AppLog << ", result = " << result << endl;
		}
	}*/
