//   $Id: PythonFilter.cpp 3814 2012-02-28 01:29:45Z jhill $
//
  ...
  mTimer( new PrecisionTime )
{
  try {
    ...
    mUseConsole = ( ipshell || !logFile.length() );
    if( mUseConsole ) OpenConsole( FILTER_ABBREV );
...
#error unfinished (see comments below)
    // TODO: export command-line args etc as dictionary or as keyword args to 'go'
    //          InstallationDir, Log, MODTYPE, Framework, UseConsole, WD, ClassFile, Shell
    // call 'go', capture output arg as mSelf.
    // NB:  CallModuleMember may or may not be useful for this.  If not, it is now unused.
                               ...
                               SignalProperties& outSignalProperties ) const
{
  try
  {
    ...
    PyObject* py_list;
    py_list = PyObject_GetAttrString( mSelf, (char*)"_writeable_params" );
...
{
#if MODTYPE == 3
  State( "AppStartTime" ) = mTimer->Now();
#endif
  try
  {
    if( mSharedInputSignal )
    {
...
FILTER_NAME::StartRun()
{
  try
  {
    ...
    UpdateStateChangesFromPython( before, after );
    UnblockThreads();
    if( mSharedFlag ) *mSharedFlag = 0.0;
  }
  catch( EndUserError& e )
  {
...
FILTER_NAME::SharingSetup( const SignalProperties &inProp, const SignalProperties &outProp )
{
  ...
  {
    mSharedInputSignal  = NULL;
    mSharedOutputSignal = NULL;
    mSharedStateValues  = NULL;
    mSharedFlag         = NULL;
  }
  else
  {
    mSharedInputSignal  =                        (PyArrayObject*)PyTuple_GetItem( py_result, 0 ) ;
    mSharedOutputSignal =                        (PyArrayObject*)PyTuple_GetItem( py_result, 1 ) ;
    mSharedStateValues  = (double*)PyArray_DATA( (PyArrayObject*)PyTuple_GetItem( py_result, 2 ) );
    mSharedFlag         = (double*)PyArray_DATA( (PyArrayObject*)PyTuple_GetItem( py_result, 3 ) );
  }
  Py_DECREF( py_result );
...
FILTER_NAME::Share( const GenericSignal &inSignal, GenericSignal &outSignal )
{
  ...
  while( *mSharedFlag != 0.0 )
  {
    if( *mSharedFlag < 0.0 ) return -1;
    ::Sleep( 1 );
  }

  ConvertSignalToPyArrayObject( inSignal, mSharedInputSignal );

  for(i = 0; i < nstates; i++ )
    mSharedStateValues[i] = (double)State(  (*States)[i].Name().c_str()  );

  *mSharedFlag = 1.0;
  while( *mSharedFlag == 1.0 ) ::Sleep( 1 );
  if( *mSharedFlag < 0.0 ) return -1;

  ConvertPyArrayObjectToSignal( mSharedOutputSignal, outSignal );

  for(i = 0; i < nstates; i++ )
    State(  (*States)[i].Name().c_str()  ) = (StateType)mSharedStateValues[i];
...
FILTER_NAME::ConvertSignalToPyArrayObject( const GenericSignal& sig, PyArrayObject* array ) const
{
  ...
  if(array == NULL) array = (PyArrayObject *)PyObject_CallMethod( mSelf, (char*)"_zeros", (char*)"ii", sig.Channels(), sig.Elements());
...
FILTER_NAME::BlockThreads() const
{
  ...
  PyEval_RestoreThread( mSavedThreadState );
}
...
FILTER_NAME::UnblockThreads() const
{
  ...
  mSavedThreadState = PyEval_SaveThread();
}
...
FILTER_NAME::CallMethod( const char* name, PyObject* arg1, PyObject* arg2, PyObject* arg3 ) const
{
  ...
  PyObject* out = PyObject_CallMethodObjArgs( mSelf, py_name, arg1, arg2, arg3, NULL );
...
FILTER_NAME::CallHook( const char* name, PyObject* arg1, PyObject* arg2 ) const
{
  ...
  PyObject* py_method = PyObject_GetAttrString( mSelf, (char*)name );
  PyObject* py_wrappername = PyString_FromString( "_call_hook" );
  PyObject* out = PyObject_CallMethodObjArgs( mSelf, py_wrappername, py_method, arg1, arg2, NULL );
  HandlePythonError( name );
...
FILTER_NAME::HandlePythonError( std::string msg, bool errorCodeReturned ) const
{
  ...
  if( mSelf )
  {
    PyObject* py_error_occurred = PyObject_GetAttrString( mSelf, (char*)"_error_reported" );
    ...
    if(error)
    {
      PyObject* py_error_info = PyObject_CallMethod( mSelf, (char*)"_flush_error_info", NULL );
