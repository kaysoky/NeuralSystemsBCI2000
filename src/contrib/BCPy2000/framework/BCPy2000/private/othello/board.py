...
class Board:
    ...
    def __init__ ( self, parent=None ):
        ...
        if parent!=None:
            for i in self.r8:
                self.board.append(parent.board[i][:])
            self.black_pieces=parent.black_pieces
            self.white_pieces=parent.white_pieces
            self.valid_moves=parent.valid_moves
        else:
            for i in self.r8:
        ...
        for i in self.r8 :
            for j in self.r8 :
                if self.board[i][j] == color :
                    places.extend(self.lookup ( i, j, color ))
	...
	(1=North,2=Northeast...) from position. """
        ...
        r8=self.r8
        if  i in r8 and j in r8 and self.board[i][j] == other:
            # assures there is at least one piece to flip
            places.append((i,j))
            i = i + row_inc
            j = j + col_inc
            while i in r8 and j in r8 and self.board[i][j] == other:
                # search for more pieces to flip
                places.append((i,j))
                i = i + row_inc
                j = j + col_inc
            if i in r8 and j in r8 and self.board[i][j] == color:
