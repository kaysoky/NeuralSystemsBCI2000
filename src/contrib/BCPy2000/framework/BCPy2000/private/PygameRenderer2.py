...
class PygameRenderer(BciGenericRenderer):
	...
	def __init__(self):
		...
		self.coords = Coords.Box(left=100, top=100, width=300, height=300, sticky=False, anchor='top left')
	...
	def setup(self, left = None, top = None, width = None, height = None,
		...
		frameless_window = None, always_on_top = None, title=None, **kwds):
		"""
		...
		self.framerate = float(framerate) if framerate != None else self.framerate  # TODO: UNUSED
		...
		self.always_on_top = always_on_top if always_on_top != None else self.always_on_top
	...
	def Initialize(self, bci=None):
		...
		pygame.display.set_icon(pygame.image.load(os.path.join(Coords.__file__, '..', '..', '..', 'icon.bmp')))
		flags = \
			(self.changemode and (pygame.FULLSCREEN | pygame.DOUBLEBUF)) | \
			(self.frameless_window and pygame.NOFRAME) | 0
		self.screen = pygame.display.set_mode((int(self.coords.width), int(self.coords.height)), flags)
	...
	def StartFrame(self, objlist):
		...
		for obj in objlist: obj.draw(self.screen)
...
class Text(ImageStimulus):
	...
	def transform(self, force=False):
		...
		if p['font_changed']:
			...
			if fn != None: p['font_object'] = pygame.font.Font(fn, p['font_size'])
			p['font_changed'] = False
			p['text_changed'] = True
		if p['text_changed']:
			t = str(p['text'])
			if p['value'] != None:
			...
			p['original'] = orig = p['font_object'].render(t, True, (255,255,255))
			self.size = Coords.Size((orig.get_width(), orig.get_height()))
			p['text_changed'] = False
			p['changed'] = True
		return ImageStimulus.transform(self, force=force)
	
	@apply
	def value():
		def fget(self):  p = self.__dict__['__props']; return p['value']
		def fset(self, val):
			if isinstance(val, (tuple,list,numpy.ndarray)): val = list(val)
			p = self.__dict__['__props'];
			p['text_changed'] = p['value'] != val
			p['value'] = val
		return property(fget, fset, doc='optional list of values for interpolation into text')
		
	@apply
	def text():
		def fget(self):  p = self.__dict__['__props']; return p['text']
		def fset(self, val):
			if val == None or val == '': val = ' '
			p = self.__dict__['__props'];
			p['text_changed'] = p['text'] != val
			p['text'] = val
		return property(fget, fset, doc='text content')
		
	@apply
	def font_name():
		def fget(self):  p = self.__dict__['__props']; return p['font_name']
		def fset(self, val):
			p = self.__dict__['__props'];
			p['font_changed'] = p['font_name'] != val
			p['font_name'] = val
		return property(fget, fset, doc='font name')
		
	@apply
	def font_size():
		def fget(self):  p = self.__dict__['__props']; return p['font_size']
		def fset(self, val):
			p = self.__dict__['__props'];
			p['font_changed'] = p['font_size'] != val
			p['font_size'] = val
