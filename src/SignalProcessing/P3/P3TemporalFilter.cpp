...
TemporalFilter::TemporalFilter(PARAMLIST *plist, STATELIST *slist)
{
  ...
  //      tempfile= fopen("tempfile.asc","w+");
        ...
        strcpy(line,"Filtering float StopMem= 30.0 30.0 0.0 512.0   // End of Spectrum in Hz");
        plist->AddParameter2List(line,strlen(line) );
        strcpy(line,"Filtering float deltaMem= 0.2 0.2 0.02 2.00    // Resolution (line density)");
        plist->AddParameter2List(line,strlen(line) );
        strcpy(line,"Filtering float MemBandWidth= 3.0 3.0 0.5 32.0 // Spectral Bandwidth in Hz");
        plist->AddParameter2List(line,strlen(line) );
        strcpy(line,"MEM int MemModelOrder= 10 10 2 32    // v20d");
        plist->AddParameter2List(line,strlen(line) );
        strcpy(line,"MEM int MemWindows= 2 2 1 8          // AR- number of input blocks");
        plist->AddParameter2List(line,strlen(line) );
        strcpy(line,"Filtering int MemDetrend= 0 0 0 2              // Detrend data?  0=no 1=mean 2= linear");
        plist->AddParameter2List(line,strlen(line) );
...
static count= 0;
 ...
 // actually perform the Temporal Filtering on the input and write it into the output signal
    ...
    //    fprintf(tempfile,"%5d ",count);
        ...
        {
                for(j=1;j<datawindows;j++)
                {
                        for(k=0;k<input->MaxElements;k++)
                        {
                                ncount= j*k;
                                ocount= (j-1)*k;
                                datwin[ncount]= datwin[ocount];
                        }
                }

                for(j=0;j<input->MaxElements;j++)
                        datwin[j]= input->GetValue(i,j);


                mem->setData( winlgth, datwin );
                mem->get_mem();
